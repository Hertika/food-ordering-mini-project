...
class CustomerHomePage(BackgroundPage):
    ...
    def add_menu(self):
        ...
        self.menu_frame.place(x=700, y=170)
    ...
    def order_confirmation(self, result):
        ...
        f = SimpleTable(confirmation_window, rows=len(result), columns=len(result[0])+1, height=300, width=500)
        f.pack()
        ...
        for r in range(len(result)):
            for c in range(len(result[0])):
                ...
                if r==0:
                    f.set(row=r, column=c+1, value="Quantity")
                else:
                    quan = IntVar()
                    result[r].append(quan)
                    check_b = Spinbox(f, from_=0, to=10, font=self.text_font,
                                      textvariable=quan)
                    f.set(row=r, column=c+1, value=quan, widget=check_b)
    ...
    def send_order_to_admin(self, result, confirmation_window):
        ...
        total_price = map(lambda x: x[1] * x[2].get(), result[1:])
    ...
    def add_menu_items(self, food_type):
        ...
        for r in range(len(result)):
            for c in range(len(result[0])):
                if c==0 and r!=0:
                    check_b = Checkbutton(self.menu_items_frame, text=result[r][c], font=self.text_font, variable=self.dct_IntVar.get(result[r][1]))
...
if (__name__ == "__main__"):
    root = Tk()
    details = {'CustomerId': 1, 'CustomerName': 'batra', 'CustomerPassword': 'batra', 'CustomerContact':7709431010,
               'CustomerEmailId': 'batra@gmail.com'}
